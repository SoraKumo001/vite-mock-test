    ...
    simple(ast, {
        AssignmentExpression(node, state) {
            if (node.left.type === "MemberExpression" &&
                ...
                if (node.left.object.name === "module" &&
...
const convertPrivate = (ast) => {
    ...
    const imports = ast.body.filter((node) => outsides.includes(node.type) ||
...
export const viteMockPlugin = (props) => {
    ...
    return {
        ...
        load(id) {
            if (id === virtualMockName) {
                const code = fs.readFileSync(path.resolve(__dirname, "___mock.js"), "utf-8");
                return code;
            }
        },
        transform(code, id) {
            ...
            const p = path.relative(path.normalize(path.resolve("./")), path.normalize(id.replaceAll("?", "-").replaceAll("\0", "")));
            const name = p
                .replaceAll("/", "-")
                .replaceAll("\\", "-")
                .replaceAll(":", "-")
            ...
            try {
                ...
                if (Object.keys(exports).length) {
                    ...
                    if (Object.keys(exports).length) {
                        const insertMockAst = parse(`import {} from "${virtualMockName}";
            ...
            `, {
                        ...
                        convertPrivate(ast);
                        const exportAst = parse((namedExports.length
                ...
                if (___DEBUG) {
                    fs.writeFileSync(`tmp/${name}-error.json`, JSON.stringify(ast, null, 2));
                }
                console.log("==============================");
                console.log(id);
                console.log(e);
